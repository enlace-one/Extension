...
document
  .getElementById("reset-page-note-config")
...
document
  .getElementById("reset-all-settings-json")
  .addEventListener("click", function () {
...
document
  .getElementById("save-all-settings-json")
  .addEventListener("click", function () {
    try {
...
const rotateClipboxEncryption = async () =>
  await manageEncryption({ operation: "rotate", target: "clipbox" });
const rotatePageNotesEncryption = async () =>
  await manageEncryption({ operation: "rotate", target: "pagenotes" });
const removeClipboxEncryption = async () =>
  await manageEncryption({ operation: "remove", target: "clipbox" });
const removePageNotesEncryption = async () =>
  await manageEncryption({ operation: "remove", target: "pagenotes" });
const addClipboxEncryption = async () =>
  await manageEncryption({ operation: "add", target: "clipbox" });
const addPageNotesEncryption = async () =>
  await manageEncryption({ operation: "add", target: "pagenotes" });
const rotateAllEncryption = async () =>
  await manageEncryption({ operation: "rotate", target: "both" });
...
document
  .getElementById("reset-page-notes")
  .addEventListener("click", function () {
    deleteAllPageNotes();
  });

document
  .getElementById("reset-clipboard")
  .addEventListener("click", function () {
...
document
  .getElementById("regexCode")
  .addEventListener("keydown", function (event) {
    if (event.key === "Enter") {
      event.preventDefault();
      const regexCode = new RegExp(document.getElementById("regexCode").value);

      Array.from(document.getElementsByClassName("regexMatch")).forEach(
        (element) => {
      ...
      Array.from(document.getElementsByClassName("regexFail")).forEach(
        (element) => {
          if (regexCode.test(element.value)) {
...
document
  .getElementById("replacer-switch")
  .addEventListener("click", function () {
...
document
  .getElementById("replacer-regex-switch")
  .addEventListener("click", function () {
...
function toTitleCase(str) {
  return str.toLowerCase().replace(/(^|\s)\w/g, (l) => l.toUpperCase());
}
...
function toCamelCase(str) {
  return str
    ...
    .replace(/[^a-zA-Z0-9]+/g, " ")
    .replace(/(^|\s)\w/g, (m, o) =>
      o === 0 ? m.trim().toLowerCase() : m.trim().toUpperCase()
    )
    .replace(/\s+/g, "");
}
function convertCase() {
  const input = document.getElementById("case-changer-input").value;
  const sel = document.getElementById("case-changer-select").value;
  let output = "";
  switch (sel) {
    case "lower":
  ...
  document.getElementById("case-changer-output").value = output;
}
document.addEventListener("DOMContentLoaded", function () {
    // Auto-convert on change
    document
    .getElementById("case-changer-input")
    .addEventListener("input", convertCase);
    document
    .getElementById("case-changer-select")
    .addEventListener("change", convertCase);

    document.getElementById("case-changer-copy-output").addEventListener("click", function () {
        const element = document.getElementById("case-changer-output")
        copyValue(element)
    })
})
//////////
...
if (isUserScriptsAvailable()) {
  document
    .querySelectorAll(".hidden-if-not-developer-mode")
    .forEach((element) => element.classList.remove("hidden"));
  document
    .querySelector('.scriptName[data-tab="script-tab"]')
    .classList.remove("hidden");
  document
    .querySelector('.regexName[data-tab="regex-tab"]')
    .classList.remove("hidden");
  document
    .querySelector('.htmlName[data-tab="html-tab"]')
    .classList.remove("hidden");
  updateUi();
}
...
document
  .getElementById("delete-expired-page-note-button")
  .addEventListener("click", function () {
    showNotification("Deleted expired page notes");
  });

document
  .getElementById("open-url-pattern")
  .addEventListener("click", function () {
